...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
  if (huart->Instance == USART2)
  {
    ...
    uint8_t recibido = 'R';
    HAL_UART_Transmit(&huart2, &recibido, 1, 10); // Enviar el byte recibido de vuelta
...
// Habilitar la recepción UART por interrupción por primera vez
  ...
  while (1)
  {
    ...
    if (HAL_GetTick() - last_heartbeat_time >= 100) // Cada 5000 ms
    {
    ...
    if (button_flag == 1)
    {
      ...
      sprintf(tx_buffer, "Boton azul activado! LED externo encendido.\r\n");
    ...
    if (led_ext_off_time != 0 && HAL_GetTick() >= led_ext_off_time)
    {
      ...
      sprintf(tx_buffer, "LED externo apagado.\r\n");
      HAL_UART_Transmit(&huart2, (uint8_t*)tx_buffer, strlen(tx_buffer), 100);
